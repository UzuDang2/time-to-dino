...
`ActiveQuestHUDPanel.top: 110px` → `80px` (D-141 SUPERSEDED). 상단 ui-panel과 살짝 겹치는 시각이지만 요한 판단상 110은 너무 내려가 있었음.

---

## D-154. 퀘스트 목록 — 패널 자체를 버튼화, 모달 + 행 클릭 → 상세 (2026-04-27, `index.html`)

요한 원문: "퀘스트 목록은 그자체로 버튼이고, 퀘스트목록 버튼을 누르면 퀘스트 목록을 보여주는 팝업창이 떠야해, 나중에 퀘스트 항목이 많아질테니 스크롤 구현해주고, 바로 받기 버튼이 있는게 아닌 패널 자체를 누르면 그때 퀘스트 내용을 보여주는 팝업이 떠서 거기서 수락을 누르는거야".

### 결정

D-148/D-151의 인라인 [받기] 버튼 패턴 SUPERSEDED — 패널 자체가 버튼 → 모달 → 행 클릭 → 상세 모달 흐름.

#### 캠프 메인

- 인라인 "퀘스트 목록" 섹션 + "진행 중인 퀘스트" 섹션 → **단일 [퀘스트 목록] 버튼**.
- 버튼 우측에 요약: "받기 가능 N · 진행 중 M · 완수 K".

#### QuestListModal

- 받기 가능 / 진행 중 / 완수 행을 한 모달에 스크롤 노출(`maxHeight: 80dvh`, 자체 `overflowY: auto`).
- 받기 가능 행 클릭 → `setOfferQuestId(def.id)` → 기존 TentQuestModal (스토리·정보·[수락]).
- 진행 중 행 클릭 → `setActiveDetailQuestId(quest.id)` → **새 진행 상세 모달** (`renderQuestCard` 재사용 — 재료 진행 + [완수] 버튼).
- 완수 행은 클릭 비활성, opacity 0.7.

### 효과

향후 퀘스트 항목 다수 추가돼도 캠프 메인 화면 점유 일정. 모든 퀘스트 흐름이 "행 클릭 → 상세 → 행동" 패턴으로 통일.
...
방어구 데이터는 이미 단계별 defense 보유 (잎사귀=1, 2단계=2, 3단계=3). D-73이 `shield_block` 카드만 인벤 방어구 합산을 받게 제한 → 사용자는 일반 방어 카드(crouch=웅크리기 등)도 같은 효과 원함. 추가로 카드 사용 시 +1 보너스.

### 결정 (D-73 일반화)

`combatDeck.js` build 단계에서 `cardDefense > 0`인 모든 카드(요구 있음/없음 모두):
```
finalDefense = cardDefense + shieldSum + armorSum + 1
```
- 무한카드 분기(crouch 등 reqs.length === 0)도 동일 처리.
- D-73의 `card.id === 'shield_block'` 분기 제거 — 일반화.
- 방어구 데이터는 변경 없음(잎사귀 1·2단계 2·3단계 3 그대로).

### 효과

- 잎사귀(1) 입고 웅크리기(card 1): 1+1+0+1 = **3 방어**.
- 강화 방패(3) + 비늘갑옷(3) + 방패막기(card 1): 1+3+3+1 = **8 방어**.
- 아무 방어구 없이 웅크리기: 1+0+0+1 = **2** (기존 1 → 2, "+1 보너스" 그대로 적용).
- 카드 defense 0인 카드(주먹/돌던지기 등)는 영향 없음.

### 검증

- 잎사귀 입고 crouch 사용 시 prey 공격 1-3 데미지가 max(0, atk-3)으로 감쇄.
- shield_block의 누적 방어 효과는 D-73 의도 보존(추가 +1만 더해짐).
...
`ActiveQuestHUDPanel` `position: fixed; top: 12px; right: 12px`. 게임 화면 상단 ui-panel(2줄 HUD ≈ 90px)과 겹쳐 발각률 바·시계 등 stat-box를 가림.

### 결정

`top: 12px` → `top: 110px`. ui-panel 90px + 여유 20px. 맵 영역 우상단 모서리에 자리 — 다른 UI 가림 없음.

---

## D-140. L1 prey 포식 시간 2턴 (L2 3턴 유지) (2026-04-27, `boss.js`)

요한 원문: "레벨1타입의 포식은 2턴만 휴식하게 해줘".

### 결정

`boss.js` 포식 시작 분기에서 `this.predationStay = 3` → `(target.level === 2) ? 3 : 2`. L1은 작은 먹이라 빠르게 끝남.

### 영향

- L1 사냥감 미끼 활용 시 안전 시간 2턴(보스 도착 턴 포함하면 약 2턴 정지).
- L2는 D-90 그대로 3턴.
- target.level이 명시 안 되면 L1로 간주(보수적 짧은 stay).

---

## D-139. 보스 포식 후보 — L1 포함 모든 prey (2026-04-27, `boss.js`)

요한 원문: "죽은게 말이 안되는걸? 사냥감이 살아있는칸이었고 보스가 왔다면 포식중이었어야 하는데 내가 왜 죽었지? 포식 먼저 발생하게 처리 된거 맞아?"

### 진단 (사용자 스크린샷 기반)

덤불 타일에 토끼(L1) 살아있고 보스가 와서 게임 오버. 코드를 보니 `boss.js` 포식 후보 필터에 `if (p.level != null && p.level !== 2) return false`가 있어 **L1 prey는 포식 대상이 아니었음**. 따라서:
- L1 prey 타일에 보스 도착 → 포식 트리거 안 됨 → `inPredationSafe = false` → `encounter && !inPredationSafe` → 게임 오버 분기.

이는 D-77 도입 당시 디자인이었지만 사용자 멘탈 모델(사냥감 = 미끼)과 어긋남.

### 결정 (D-77 부분 SUPERSEDED)

포식 후보 필터에서 `level !== 2` 가드 제거. L1·L2 가리지 않고 거리 2 이내 모든 prey가 포식 타겟. L1 사냥감 타일에서도 보스 도착 시 포식 시작 → 플레이어가 같은 타일에 있어도 D-93 inPredationSafe 분기로 안전.

### 효과

- **L1 prey도 보스 미끼로 활용 가능** — 토끼·메추라기 등 발견 시 보스가 추적해 포식 시작, 플레이어는 그 사이 안전 통과.
- 포식 시간(stay=3턴)은 그대로 — L1·L2 동일 처리.
- 사냥 모달(hunt_start)은 보스 이동 후 push되므로 포식 시작 케이스에서는 hunt_start 카드 추가 안 됨(`!inPredationSafe` 가드 D-93 그대로 동작).

### 검증

- L1 사냥감 타일 + 보스 도착 시 게임 오버 대신 "거대한 짐승이 식사에 정신이 팔려있다" 토스트 + msg 메시지 노출.
- L2 사냥감 동작은 변동 없음.

---

## D-138. L2 사냥감 일반·특수 공격 최소 명중 1 보장 (2026-04-27, `data/prey_actions.json` / `data/data.js`)

요한 원문: "레벨2등급의 사냥감들이 사용하는 모든 특수 공격에 최소 명중 1을 해줘 이미 가진 수치가 있었다면 건들지 말고, 레벨 2등급의 사냥감들이 사용하는 일반 공격에도 최소 명중 1을 붙여붜".

### 결정

L2 사냥감(boar/dinosaur/deer/badger/fox/turkey/armadillo) 7종이 사용하는 attack 타입 액션 중 accuracy 0인 항목을 1로 인상:
- `attack` (일반 공격, L2 평타): 0 → 1
- `frenzy_claw` (오소리 특수): 0 → 1
- `wing_slap` (큰 새 특수): 0 → 1

기존 1+ 인 항목은 그대로 보존:
- `tusk_charge` (멧돼지) 1, `pounce_bite` (작은 공룡) 1, `gore` (사슴) 1, `feint_strike` (여우) 2.

L1 전용 attack 액션(kick/nibble/acorn_throw/peck/slime_lash/venom_bite/tongue_lash/pinch/kick_jump)은 변경 없음 — 사용자가 "L2"만 명시.

### 왜

D-96 정수화 이후 accuracy 0은 evade 1을 못 뚫어 전투가 의도와 다르게 결정론적으로 흘러감(L2 사냥감이 모든 1+ 회피에 명중 못함). L2는 위계상 위협적이어야 하므로 모든 attack에 최소 1 명중 부여 — evade 1과 정면 격돌 가능.

### 검증

- 사슴/멧돼지 등 기존 1+ 명중 사냥감의 디자인 보존.
- 오소리·큰 새가 evade 1 보유 캐릭터에게도 명중 가능.
- L1 사냥감은 영향 없음 (L1 evade=1 정책 D-102 유지).
...
# D-249 v2 — 공격성 사냥감 선공: HP 차감 → 자동 hunt 진입으로 정정 (2026-05-06)

## 결정

D-249 v1(진입 즉시 setHealth -prey.attack + ‼️ 내레이션) **SUPERSEDED**. "선공"의 진짜 의미는 "사냥시작 버튼을 누른 것과 동일" — aggressive=true 사냥감 칸 진입 시 자동으로 hunt 모드(전투 화면) 진입. 데미지 직접 차감 없이 첫 턴부터 회피/방어/공격 카드 정상 사용 가능. 보스=공포 원칙과 일관 (강제 전투 자체가 위협).

## 구현

- **index.html (moveTo preyHere 분기)**: `ambushDamage` / `ambushPreyName` 산출 폐기 → `aggressivePreyAuto = { prey, def }` 페이로드만 마련. aggressive면 HUNT_START_CARD 손패 추가도 skip(중복 방지).
- **preyNarration 변경**: aggressive 자동 진입 시 `‼️ {이름}이(가) 풀숲을 박차고 달려든다! 사냥이 시작된다.` (HP 차감 멘트 제거, 자동 진입 사실만 알림).
- **moveTo 끝**: setMessage 후 aggressivePreyAuto 있으면 `setActiveHunt({ prey: {...} })` 직접 호출 — useCard('hunt_start')의 페이로드 그대로 복제.
- **시간/발각 추가 차감 없음** — 사용자 의도는 "거부할 수 없는 만남"이라 카드 비용까지 떠넘기는 건 가혹. 이번 이동분만 차감.
- **wait-in-place / inPredationSafe 미적용** 정책 유지.

## 검증

- preview reload 후 콘솔 에러 없음 (BABEL deopt 노트만, 정상).
- 정적 grep: `ambushDamage` 잔재 0곳, `aggressivePreyAuto` 4곳, `D-249 v2` 2곳.
- 인게임 라이브 검증(실 boar/dinosaur 진입)은 요한 수동 QA에 위임.

## 파일

- `index.html:9605~9645` (preyHere 분기 v2 — 자동 진입 페이로드)
- `index.html:9712~9714` (ambushNarration 제거 자리 주석)
- `index.html:9750~9790` (composeNarration + setActiveHunt 자동 진입 트리거)

---

# D-250 — 도망치기 시스템: 즉발 → 카드 슬롯 추가형 (2026-05-06)

## 결정

D-225 즉발 도망(버튼 → 즉시 success_rate 굴림 + 실패 시 1.5s 메시지) **SUPERSEDED**. 새 정책: 버튼을 누르면 빈 슬롯에 도망치기 카드 1장이 추가되고, confirm 시 combatDeck.resolveHunt가 슬롯 순서대로 굴려 한 장이라도 성공(20%)하면 player_fled outcome. 회피/방어/공격 카드와 자유 조합 가능.

## 메커니즘

- **무한 사용** — `infinite:true`라 빈 슬롯이 있는 한 계속 추가 가능. 3턴 전투에서 3번 누르면 3슬롯에 다 도망치기 카드 = 1-(0.8)^3 = 48.8% 성공.
- **20% success_rate** — 시트 `전투카드.run_away` row의 success_rate 25 → 20 (`--sheet-op update-cell`).
- **실패 메시지 제거** — 결과는 confirm 후 turn 로그(`도망 시도 실패` 라인)에서 확인. 즉발이 아니므로 별도 1.5s 메시지 불필요.

## 구현

- **시트 갱신**: `python3 scripts/fetch_data.py --sheet-op update-cell --tab 전투카드 --match-col id --match-val run_away --set-col success_rate --set-val 20` → `make data`.
- **index.html handleRunAway**: 즉시 발동 로직 폐기 → `slots.findIndex(null)`로 빈 슬롯 찾아 runAwayCard 1장 push. 슬롯 가득이면 noop.
- **runAwayMsg state·setRunAwayMsg / 1.5s timeout** 제거.
- **BattleStage prop**: `runAwayMsg` 제거, `slotsFull` 추가 (가득 시 버튼 disabled + 색상 톤 다운).
- **resolveHunt 처리**: 기존 코드 그대로 활용 — `card.id === 'run_away' && cardHit` 분기에서 outcome='player_fled' break.

## 검증

- 시뮬: 4슬롯 도망치기 × 2000회 → 1222 fled / 778 prey_fled / 0 victory. 기대 59% ≈ 실측 61%, 통계적 정상.
- preview reload 콘솔 에러 없음.
- data.js `run_away.success_rate` = 20 확인.

## 파일

- `data/combat_cards.json` (auto-generated, success_rate 25→20)
- `data/data.js` (browser bundle, auto-generated)
- `index.html:6237~6240` (BattleStage prop: runAwayMsg→slotsFull)
- `index.html:6474~6490` (도망치기 버튼 — disabled 분기)
- `index.html:6957~6973` (handleRunAway — 슬롯 추가형)
- `index.html:7228~7230` (BattleStage 호출 prop)

## D-275. 연속 카드 보너스 +1 — 콤보 시스템 (2026-05-08)

**의도** (요한): 전투를 더 재밌게. "같은 카드를 연속으로 낼 경우 효과가 +1 강화" + 유저가 보너스를 한눈에 알 수 있는 시각 표시.

## 결정

- **트리거**: 직전 슬롯(t-1)과 현재 슬롯(t)의 카드 `id`가 같고 둘 다 카드가 있으면 콤보 발동. 첫 슬롯(t=0)은 콤보 불가.
- **+1 적용 대상**: 카드의 핵심 스탯 1개. 우선순위 `damage > defense > evade > accuracy` (한 카드는 보통 핵심 스탯 1개).
- **누적 정책**: "직전 비교" 방식 — 3연속이면 t=1, t=2 각각 +1. 누적 +2가 아닌 **연속 한 단계당 +1**. 단순·직관성 우선.
- **리셋**: 다른 카드 끼면 자동 리셋(직전 비교라 별도 로직 불필요).
- **무기 카드 동일 적용**: id가 같으면 OK. 화살쏘기 2연속 → 명중/대미지 +1.
- **시각 큐**: 슬롯 카드 좌상단 골드 배지("콤보 +1") + 슬롯 wrapper 골드 outline + 글로우. 턴 로그에는 "연속 +1" 골드 chip 부기.

## 구현

- **`combatDeck.js::resolveHunt`**:
  - 매 turn 시작에서 `prevCard = userSlots[t-1]`과 비교 → `comboBonus = { stat, amount: 1 }` 생성.
  - `cardDamageWithCombo` / `cardAccuracyWithCombo` / `cardDefense` / `cardEvade` 로컬 변수에 보너스 합산. 원본 card 변경 X.
  - turn 결과(`turns[].combo`, `turns[].comboBonus`) 부착 — UI가 사후 표시 가능.
- **`index.html` BattleStage 슬롯 렌더(block 3)**:
  - 직전 슬롯과 같은 id이면 `comboStat` 계산(SSOT 동일 우선순위).
  - wrapper outline `#ffcf4a` + box-shadow 골드 글로우.
  - 좌상단 absolute 배지("콤보 +1") — 그라디언트 골드, 진갈색 텍스트, z-index 5.
- **`index.html` 턴 로그**: `log.combo` 시 카드명 라인 옆에 "연속 +1" 골드 chip.

## 검증

- 단위 검증 (`preview_eval`):
  - 3연속 punch → T1 1dmg / T2 2dmg combo / T3 2dmg combo. ✓
  - punch / dodge / punch → 모두 콤보 X (다른 카드로 리셋). ✓
- preview reload 콘솔 실에러 0 (BABEL deopt 경고만 — 기존 단일 파일 크기 이슈).
- BABEL pass.

## 파일

- `combatDeck.js:374~404` (resolveHunt — comboBonus 계산 + 합산값 변수)
- `combatDeck.js:524~530` (turns[].combo / comboBonus 부착)
- `index.html:7318~7368` (BattleStage block 3 — 콤보 outline·글로우·배지)
- `index.html:8158~8175` (턴 로그 "연속 +1" 부기)

## D-276. L2 사냥감 행동패턴에 "알 수 없음(❓)" 슬롯 도입 (2026-05-08, 시트 + `combatDeck.js` + `index.html`)

요한 원문: "2등급 사냥감 행동 시퀀스 중 일부를 유저에게 알 수 없음으로 표시. 실제 동작은 그 사냥감의 기존 행동패턴 중 하나가 그대로 실행." 기획 의도: 유저가 미리 적의 다음 수를 100% 읽지 못하게 만들어 긴장감 부여.

## 결정

- **L2 7종 모두 turn3 슬롯을 `?` 토큰으로 통일**. (boar/dinosaur/deer/badger/fox/turkey/armadillo)
  - 4슬롯 구조에서 가운데-후반: "준비 → ❓ → 마무리" 흐름이 자연스럽고 reveal 시점에 긴장이 정점.
- **L1엔 적용 안 함** — L1은 기존대로 완전 공개. 신비성은 L2 이상의 차별화 포인트.
- **추첨**: turn 진입 시 그 prey의 `actions_per_turn`에서 `?`·빈값을 제외한 토큰 풀 균등 랜덤(`Math.random`).
  - 풀 빈 경우 `peek` 폴백(사냥감이 멈칫 — 안전한 기본).
- **slot 인덱스 통일**의 단조로움 vs 다양성 — 단조로움이 낫다는 판단(L2 첫 도입 + 단순한 학습 곡선).

## 구현

- **데이터 시트 (사냥감 탭)**: turn3 컬럼을 7종 모두 `?`로 update. `python3 scripts/fetch_data.py --sheet-op=update-cell` 7회. `make data` 후 `data/prey.json`·`data/data.js` 동기화.
- **`combatDeck.js::findPreyAction`**: `?`/`unknown` 토큰 → `{ id:'?', name:'알 수 없음', type:'unknown', ... }` sentinel 반환. UI 미리보기 표시용.
- **`combatDeck.js::rollUnknownAction(prey)`**: 신규 helper. prey의 비-`?` 토큰 풀에서 균등 추첨 → `findPreyAction`으로 실제 actionDef 반환.
- **`combatDeck.js::resolveHunt`**: turn 진입 시 `actionDef.type === 'unknown'`이면 `rollUnknownAction(prey)`로 교체 + `revealedFromUnknown` flag · `revealedAction` 객체를 모든 turns.push 객체에 일관 부착(`revealInfo` 변수로 통합).
- **`index.html` BattleStage 슬롯 미리보기**: `type === 'unknown'`이면 ❓ 큰 글자 + "알 수 없음" + 보라톤 배경(`#3a2a4e`) + dashed 보더(`#b890e0`). 정상 행동 슬롯과 시각적으로 분명히 구분.
- **`index.html` HuntCombatModal slotActions state화**: 기존 `useMemo` → `useState(initialSlotActions)` + prey 바뀌면 reset(`useEffect`). resolve 진행 step1(t+0) 시점에 `turn.revealedFromUnknown && turn.revealedAction`이면 `setSlotActions(prev → next[idx]=revealedAction)`로 해당 슬롯 swap.

## 검증

- 시트 update + `make data` 정상 → `prey.json`의 7종 모두 turn3 `?` 확인. ✓
- preview reload(캐시 버스트 `?v=Date.now()`) 후 `findPreyAction('?')` → unknown sentinel 반환. ✓
- `resolveHunt` 단위 sim: boar/fox/armadillo/dinosaur 각 300회 → revealedTurns 300/300, distribution 균등(~33%씩 3가지 행동). rabbit(L1)은 revealed 0. ✓
- HuntCombatModal 직접 mount 스크린샷: T3 슬롯 ❓ + 알 수 없음 + 보라 dashed 보더 시각 확인. T1/T2/T4는 정상 행동. ✓
- BABEL pass, 콘솔 실에러 0 (기존 BABEL deopt 경고만).

## 파일

- `data/prey.json` (L2 7종 turn3 `?` — 시트 동기화 결과)
- `combatDeck.js:228~282` (findPreyAction + rollUnknownAction)
- `combatDeck.js:412~424` (resolveHunt unknown reveal + revealInfo 변수)
- `combatDeck.js:515~620` (모든 turns.push에 ...revealInfo 부착)
- `index.html:7575~7600` (slotActions state + reset effect)
- `index.html:7113~7152` (BattleStage 슬롯 unknown 분기 — ❓·보라 dashed)
- `index.html:7868~7886` (resolve step1 setSlotActions reveal swap)

## D-277. 방어 카드 2턴 지속 시인성 3중 큐 (2026-05-08, `combatDeck.js` + `index.html` + `gameStyles.css`)

요한 원문(D-276 이후 후속): 방어 카드(crouch / shield_block 등 `card.defense > 0`)가 발동 turn + 다음 turn까지 2턴 흡수하는데 UI에 지속 정보 0 → "이번 턴만 막힌다"고 오해. 시인성 3중 큐로 보완.

## 결정

- **카드면 "2턴" chip** — 손패·슬롯 양쪽 일관 노출. `def` 스탯 컬럼 아래 파란 그라데이션 chip.
- **슬롯 N+1 carry 오버레이** — N에 defense 카드 배치 시 N+1 wrapper에 파란 dashed outline + 좌하단 "🛡️ 지속" 미니 배지. 빈 슬롯이어도 표시. N+1에 자체 defense 카드 깔리면 carry 큐 숨김(자기 stack 우선).
- **"나" 박스 라이브 stack 카운트** — 정적 `🛡️ {cap}` → 동적 `🛡️ {liveStack}/{cap}`. 활성 시 파란 글로우 펄스(0.6s alternate). 만료 시 0으로 복귀.

## 구현

- **`combatDeck.js::resolveHunt`**: 기존엔 `lastTurn.defenseStack` 부착이 normal 흐름에만 있고 `if (!card)` continue / autoFail continue 분기에서 누락 → 빈 슬롯 turn에 `defenseStack: undefined`. 두 분기 모두 turn 끝 직전 `lastTurn.defenseStack/evadeStack` + `stackEvents.push` 추가. 모든 turn에 stack 스냅샷 보장.
- **`index.html` HuntCard**: `card.defense > 0`이면 카드 좌측 컬럼(`def` 스탯 아래 영역)에 `<div class="hunt-card-defense-duration">2턴</div>` 노출. 손패·슬롯·preview 모두 같은 컴포넌트라 자동 일관.
- **`index.html` BattleStage 슬롯 그리드**: `prevDefenseVal > 0 && ownDefenseVal === 0`이면 `showDefenseCarry`. wrapperStyle outline 우선순위 isCurrent > isCombo > showDefenseCarry. carryBadge(좌하단 "🛡️ 지속")는 빈/채운 슬롯 양쪽 wrapper에 삽입.
- **`index.html` HuntCombatModal**: `liveDefenseStack` state 추가. resolve forEach 안 step1(t+0) 시점에 isPlayerDefend면 `defenseCap + cardDefenseVal`로 즉시 set. step2(t+200) 시점에 `turn.defenseStack`(post-absorb 스냅샷)로 갱신. `setPhase('done')` 시 0 리셋. BattleStage prop으로 전달.
- **`index.html` BattleStage 배지**: `liveDefenseStack > 0`이면 className `defense-stack-active` + 색·굵기 강조. title `2턴 흡수` 문구 추가.
- **`gameStyles.css`**: `.hunt-card-defense-duration` 파란 그라데이션 chip 스타일 + `@keyframes defense-stack-pulse` + `.defense-stack-active` 클래스(0.6s alternate infinite).

## 검증

- preview reload(`?t=${Date.now()}` 캐시 버스트) 후 `CombatDeck.resolveHunt.toString().includes('D-277')` true. ✓
- L2 boar(4턴 attack 연속) + crouch 슬롯1만, defenseCap 3 unit sim → turn1 defenseStack 2(흡수 후), turn2 defenseStack 0(흡수 후 만료), turn3·4 stack 0 + 각 2 dmg 통과. 4턴 총 4 dmg taken. ✓
- 임시 DOM 인젝션 시각 검증: "2턴" chip(파란 그라데이션 + #eaf3ff 텍스트, 7.8px) + 🛡️ 2/3 라이브 배지(animation: defense-stack-pulse 0.6s alternate infinite) 정상 적용. ✓
- 콘솔 실에러 0, BABEL pass.

## 파일

- `combatDeck.js:513~528` (`!card` continue 분기 stack 스냅샷 추가)
- `combatDeck.js:545~561` (autoFail continue 분기 stack 스냅샷 추가)
- `index.html:7449~7515` (HuntCard `<div class="hunt-card-defense-duration">2턴</div>` 추가)
- `index.html:7333~7415` (슬롯 그리드 `showDefenseCarry` + carry outline + `carryBadge`)
- `index.html:7611~7616` (HuntCombatModal `liveDefenseStack` state)
- `index.html:7889~7910` (resolve step1 setLiveDefenseStack — defense 카드 발동 시)
- `index.html:7914~7921` (resolve step2 setLiveDefenseStack — turn.defenseStack 동기화)
- `index.html:7975` (phase done 시 0 리셋)
- `index.html:8109~8112` (BattleStage prop 전달)
- `index.html:7012~7017` (BattleStage signature `liveDefenseStack`)
- `index.html:7314~7325` ("나" 박스 배지 라이브 표시 + className)
- `gameStyles.css:1015~1043` (`.hunt-card-defense-duration` + 펄스 keyframes + `.defense-stack-active`)

## D-278. 나무쑤시개 합성법 복원 (2026-05-10, `data/combos.json` + `data/data.js` + `inventory.js` + `index.html`)

요한 원문: "원래 나무쑤시개 만들 수 있었는데 언젠가부터 못하게 되었어. 배울수도 없고."

## 결정

- D-249(2026-05-06) 시트 동기화 sweep에서 `stem + wood + stone → wood_skewer` 조합 레시피가 시트 `조합레시피` 탭에서 빠지면서 `combos.json`에서도 사라졌다. 같은 자리에 D-227 워크샵 슬롯 시스템(storage 빌딩 stage 4 unlock)이 있었지만 `data/buildings.json`의 storage는 stage 1~3까지만 정의돼 stage 4 도달 자체가 불가 → 두 경로 모두 막힌 사문화 상태.
- index.html 주석에도 "wood_skewer는 combos.json 미존재라 사문화"라고 명시돼 있어 의도된 결과가 아니라 동기화 누락임을 확정.
- 환경 제약: 현재 워크트리(클라우드 세션)에 `.secrets/sheets-sa.json` 없음 → 시트 직접 입력 불가. D-90 선례대로 JSON·data.js 직접 패치 + 다음 로컬 세션 시트 동기화 TODO 등록.

## 구현

- `data/combos.json`: chipped_stone 레시피 직후에 `{ingredients:[stem,wood,stone], result:wood_skewer}` 1행 복원.
- `data/data.js` COMBOS 배열: 동일 레시피 복원 — 런타임 `window.TTD_DATA.COMBOS` 소스라 즉시 작업대 학습 패널·합성 패널에 노출.
- `inventory.js::ITEMS.wood_skewer`: static 폴백에 `durability: 5` 추가 (weapons.json 미등재 상태에서 `_placeDerivedItem`이 `durabilityLeft` 정상 초기화하도록). D-227 코멘트에 D-278 복원 메모 병기.
- `index.html` RECIPE_UNLOCK_WHITELIST 주석: "사문화" 문장 → "D-278에서 복원 — 화이트리스트로 작업대 학습 패널 자동 노출 + 합성 패널 제작 가능".

## 검증

- 인벤토리에 stem/wood/stone 1개씩 보유 → 합성 패널에서 "나무쑤시개" 표시되어 제작 가능 (런타임 검증은 다음 프리뷰 reload에서 요한이 직접 확인).
- 작업대(workshop) 모달에서 wood_skewer가 RECIPE_UNLOCK_WHITELIST에 의해 항상 unlock 상태로 노출 — 학습 비용 0 (자동 무료).

## 시트 동기화 (다음 로컬 세션 작업)

- 시트 `조합레시피` 탭: `ingredient_a=stem, ingredient_b=wood, ingredient_c=stone, result=wood_skewer` 1행 추가.
- 시트 `무기` 탭: `id=wood_skewer, name=나무쑤시개, size=2x1, grade=1단계, durability=5, description="...", summary="..."` 1행 추가 (없으면 `make data` 시 wood_skewer가 weapons.json에 안 들어가 위 합성법은 result 검증 실패로 스킵된다).
- `make data` 실행 후 `data/combos.json`·`data/weapons.json` 자동 갱신 → 이번 직접 패치와 동일 결과 보존되는지 확인.

## 파일

- `data/combos.json:175~182` (1 recipe row 추가)
- `data/data.js` COMBOS 배열 (1 recipe 추가, chipped_stone 직후)
- `inventory.js:138` (durability: 5 추가 + D-278 코멘트)
- `index.html:3363~3364` (사문화 코멘트 → 복원 코멘트)

## D-279. 베이스캠프 [보관함] 버튼 라우팅 회복 (2026-05-10, `index.html`)

요한 원문: "베이스캠프에서 보관함 열면 가방옮기기 모드로 바뀌어 그게 아니고 보관함에서 조합할수 있는 기능이어야하는데 어느샌가 없어졌어."

## 결정

- D-271(2026-05-07, 베이스캠프 UX 4중 개편) 커밋 52bed4e에서 하단 버튼 행을 [기록보기][보관함][탐험 떠나기] 3개로 통합할 때, [📦 보관함] 버튼 onClick에 `setPackOpen(true)`(PackTransferModal = 가방옮기기 모달)를 잘못 와이어링했다. 직전 옆 버튼 [탐험 떠나기]의 `setPackOpen(true)` 핸들러를 복붙하면서 발생한 회귀로 추정.
- 본래 의도는 D-113 시점부터 일관됐던 `setStorageOpen(true)` → `InventoryModal` mode="storage" → 보관함 그리드 + 합성 popover. D-147/D-149/D-150 시리즈로 다듬어진 floating 합성 popover (선택 슬롯 바로 아래에 CraftPanel 띄움)도 이 경로에서만 동작.
- 가방↔보관함 전송(PackTransferModal)은 D-271 의도대로 [탐험 떠나기] 흐름 안에서 `packDeparting=true` 상태로 진입하는 경로만 유지.

## 구현

- `index.html:6121` 보관함 버튼 onClick: `() => { setPackDeparting(false); setPackOpen(true); }` → `() => setStorageOpen(true)`로 단일 호출 회복.
- 기타 state(`storageOpen`, `storageSelectedItem`)와 `InventoryModal` 마운트(line 6218~6232)는 D-271 회귀 시점부터 잔존하고 있던 dead path였음 — 그대로 살려서 즉시 복구.

## 검증

- 베이스캠프 → 하단 [📦 보관함] 클릭 → InventoryModal(`mode="storage"`) 노출. 슬롯 선택 시 floating CraftPanel popover 노출(D-150).
- 가방↔보관함 전송은 [탐험 떠나기] 클릭 → PackTransferModal + [이대로 출발!] 흐름에서 정상 진행 가능 (D-271 설계 그대로).

## 파일

- `index.html:6120~6129` ([보관함] 버튼 onClick `setStorageOpen(true)`로 회복)
