...
## D-86. 보스 이동 템포 1턴=1칸 고정 (2026-04-24, 13th 세션, `boss.js`)

**요한 원문**: "보스가 이동 하는 방식을 유저가 타일을 이동한 후에 한칸씩 이동하도록 변경해줘."

**이전 상태**:
- 일반 모드: `playerMoveCount % 2 === 0` — 플레이어 2턴마다 보스 1이동(평균 0.5칸/턴).
- 추격 모드(`detection ≥ 80`): `bossMovePhase` 0/1 교대 → 1칸, 2칸, 1칸, 2칸… (평균 1.5칸/턴).
- 포식(D-77): BFS 첫 홉 1칸(이미 1턴=1칸).

**문제**:
1. 일반 모드 "2턴에 1번"은 유저에게 비가시적 리듬이라 언제 보스가 움직일지 예측 불가.
2. 추격 모드 "2칸 돌진"은 순간적으로 붙는 속도감을 주지만, 유저가 한 턴 실수로 즉사 각까지 가는 과속 요인.
3. 모드별로 이동 규칙이 달라 결과 화면(D-53 bossMoveHistory) 해석이 어려움.
